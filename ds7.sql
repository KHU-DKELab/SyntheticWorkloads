...
vv3 = select d_date_sk, d_year from date_dim with hint(no_inline);
...
where c_customer_sk = ss_customer_sk with hint(no_inline);
ww2 = select c_customer_id,c_first_name, c_last_name, ws_net_paid, ws_sold_date_sk from :vv1, :vv4 
where c_customer_sk = ws_bill_customer_sk with hint(no_inline);
